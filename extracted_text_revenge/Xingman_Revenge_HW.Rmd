...
## Convert PDF to text 
```{r}
#Using articles downloaded from NexisUni for Asian document

#Using pdftools package. Good for basic PDF extraction
text <- pdf_text("../extracted_text_revenge/Asian_Indian_Women_NYT_LAT_USAT.PDF")

#pdf_text reads the text from a PDF file.
writeLines(text, "../extracted_text_revenge/Asian_text.txt")
...
file_path <- "../extracted_text_revenge/Asian_text.txt"
...
output_dir <- "../extracted_text_revenge/"
for (i in seq_along(documents)) {
  output_file <- file.path(output_dir, paste0("Asian_extracted_", i, ".txt"))
  writeLines(documents[[i]], output_file)
}

cat("Files created:", length(documents), "\n")

# 13 individual text documents, including 12 articles.
```
...
```{r}
Asian_index <- read_lines("../extracted_text_revenge/Asian_extracted_1.txt")

# Extract lines 18 to 87
extracted_lines <- Asian_index[18:87]
...
write_csv(final_data, "../extracted_text_revenge/final_data.csv")
...
files <- list.files("../extracted_text_revenge", pattern="*.txt") %>% 
  as.data.frame() |> 
  rename(filename = 1) |> 
...
final_data <- read.csv("../extracted_text_revenge/final_data.csv")
...
#you need the actual hard-coded path on this line below to the text
  ...
  mutate(filepath = paste0("../extracted_text_revenge/", filename))
head(final_index)
```
...
View(articles_df)
#After viewing articles_df, I see 17 lines from the index that I don't need. Cutting them 
...
write.csv(articles_df, "../extracted_text_revenge/Asian_df2.csv")

...
```{r}
SF$disposition1 <- str_replace_all(SF$disposition1, pattern=fixed('ABA'), replacement=fixed('Abated') ) # replace ABA with abated
#Again with ADM
...
Types %>% 
  distinct() # remove the duplicate ones
```
...
```{r}
SF <- SF %>% 
  mutate(yearmo = format(call_date2, "%Y-%m")) # year in 4 digits and month in 2 digits
```
...
#rsw comment - nice job
SF_nothing <- subset(SF, grepl(c("Gone|No_Dispo"), disposition1))
head(SF_nothing)
tail(SF_nothing)
...
SF_SA <- subset(SF, grepl(c("Cited|Arrest|\\bCriminal\\b|Medical"), disposition1))
View(SF_SA)

#rsw comment: I was looking for a summary table, see below
Busted <- SF %>% 
...
SF %>% 
  ...
  geom_bar(stat = "identity", show.legend = FALSE) +
  theme(axis.text.x = element_text(angle=90)) +
  labs(title = "Number of calls by year and month, San Francisco", 
       subtitle = "SF PD Service Call Data by Month 2017-2019",
       caption = "Graphic by Wu",
       y="Number of Calls",
       x="year and month")  
...
SF %>% 
  count(day_of_week) %>% 
  group_by(day_of_week) %>% 
  ggplot(aes(x = reorder(day_of_week,n), y = n, fill=n)) +
  geom_bar(stat = "identity", show.legend = FALSE) +
  theme(axis.text.x = element_text(angle=90)) +
  labs(title = "Number of calls by weekdays, San Francisco", 
       subtitle = "SF PD Service Call Data by Month 2017-2019",
       caption = "Graphic by Wu",
       y="Number of Calls",
       x="Weekdays")  
# Both the table and the graphic show that the complaints vary by day.

#rsw comment. Good, one refinement would be to have the days flow in chronological sequence, sunday --> saturday.
